...
exports.handleAPILogin = async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign({ id: result.user._id }, process.env.JWT_SECRET, { expiresIn: '1d' });
        const refreshToken = jwt.sign({ id: result.user._id }, process.env.JWT_REFRESH_SECRET, { expiresIn: '7d' });
